...
class ListMakerApp(tk.Tk):
    def __init__(self):
        ...
        self.tier4_lbl.pack(anchor='w', padx=2)
        ...
        tk.Label(self.filter_frame, text="Skip OS Folders and offline files to reduce chance of false Antivirus alerts.", font=('', 9, 'italic'), wraplength=450, justify='left').grid(row=3, column=0, columnspan=4, sticky='w', pady=(2,0))
...
# ---------------------------------------------------------------------\n"""
    ...
    def _update_ui_state(self):
        ...
        for child in self.list_frame.winfo_children(): child.config(state=list_state)
    ...
    def _fast_scandir(self, directory, recursive=True, skip_sensitive=False):
            ...
            with os.scandir(directory) as it:
                for entry in it:
                    ...
                    if recursive and is_dir and not entry.is_symlink():
                        yield from self._fast_scandir(entry.path, recursive=True, skip_sensitive=skip_sensitive)
    ...
    def logic_standard_scan(self, queue_targets, min_size, max_size, engine, dest):
        ...
        for base_dir in queue_targets:
            ...
            for entry in self._fast_scandir(base_dir, recursive, self._current_skip_sensitive):
                if self.stop_event.is_set(): break
    ...
    def _get_suggested_filename(self, extension, source_dirs):
        ...
        return f"{root_name}_{date_stamp}{mode_suffix}.{extension}"
    ...
    def handle_standard_output(self, results, source_dirs, engine):
        ...
        if self.output_csv.get():
            ...
            try:
                ...
                self.stats['generated_paths'].append(csv_path)
            except Exception as e: self.ui_queue.put(f"[!!!] CSV ERROR: {str(e)}")
        if engine == 'sql':
            try:
    ...
    def _build_analytics_text(self, targets, include_header=False):
        ...
        if include_header:
            ...
            lines.extend([" --- RUN SETTINGS ---", f" Operating Mode:        {mode_text}", f" Processing Engine:     {engine_text}", f" Destination Output:    {dest_text}", f" Skip Hidden/System:    {skip_hidden_text}", f" Skip Files < (KB):     {self.min_size_kb.get()}", f" Skip Files > (MB):     {self.max_size_mb.get()}", "", " --- GLOBAL SUMMARY ---", f" Targets Scanned:       {len(targets)} ({', '.join(targets)})", f" Total Processing Time: {elapsed:.2f} seconds"])
